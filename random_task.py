...
class RandomTaskParallelGen(object):
    ...
    def count_exit_tasks(self):
        ...
        self.existing_tasks = len([f for f in os.listdir(os.path.join(self.output_dir, "tasks")) if
                                   os.path.isfile(os.path.join(self.output_dir, "tasks", f)) and f.startswith("task")])
    ...
    def generate_trial(self, idx, fp):
        task = self.tasks[idx]
        fi = ig.FrameInfo(task, task.generate_objset())
        compo_info = ig.TaskInfoCompo(task, fi)
        compo_info.write_trial(fp, self.img_size, True)
