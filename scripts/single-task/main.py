...
parser.add_argument('--train_path', type=str, default='/Users/lucasgomez/Desktop/Neuro/Bashvian/COGEnv/COG_v3_shapenet-main/datasets/new_val_test')# Parse the argument
parser.add_argument('--val_path', type=str, default='/Users/lucasgomez/Desktop/Neuro/Bashvian/COGEnv/COG_v3_shapenet-main/datasets/new_val_test')# Parse the argument
...
parser.add_argument('--imgm_path', type=str, default='')
parser.add_argument('--insm_path', type=str, default='')
...
if __name__ == '__main__':
    ...
    img_encoder = torchvision.models.resnext101_32x8d(torchvision.models.ResNeXt101_32X8D_Weights.IMAGENET1K_V2.transforms).to(device).eval() # torch.load(args.imgm_path, map_location=device)
    model = RNN(hidden_size=args.hidden_size, img_encoder=img_encoder, device=device, max_frames=args.task_max_len).to(device)

    ins_model = AutoModel.from_pretrained('sentence-transformers/all-mpnet-base-v2').to(device).eval() #AutoModel.from_pretrained(args.insm_path).to(device).eval()
    tokenizer = AutoTokenizer.from_pretrained('sentence-transformers/all-mpnet-base-v2') #AutoTokenizer.from_pretrained(args.insm_path)
